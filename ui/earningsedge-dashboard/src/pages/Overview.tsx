...
import { ArrowRight, CheckCircle2, XCircle, BarChart3, AlertTriangle, FlaskConical } from "lucide-react";
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-300 max-w-3xl">
              ...
              Use it to answer one question: <span className="text-slate-100 font-medium">does PPO improve equity outcomes without exploding drawdown?</span>
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            {evalStatus === "ready" ? (
              ...
              <div className="inline-flex items-center gap-2 rounded-xl border border-rose-900/40 bg-rose-950/20 px-3 py-2 text-xs text-rose-200">
                ...
                Evaluation artifacts missing (fix in runs/)
              </div>
            ) : (
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-sm font-semibold text-slate-100">PPO vs Baselines</div>
              </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-sm font-semibold text-slate-100">Error Analysis</div>
              </div>
              <ArrowRight className="h-4 w-4 text-slate-500 group-hover:text-slate-300 transition" />
            </div>
            <div className="mt-2 text-xs text-slate-400">
          ...
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="text-sm font-semibold text-slate-100">Sprint 5 Plan</div>
              </div>
              <ArrowRight className="h-4 w-4 text-slate-500 group-hover:text-slate-300 transition" />
            </div>
            <div className="mt-2 text-xs text-slate-400">
          ...
          <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="rounded-2xl border border-slate-800 bg-slate-950/30 p-4">
              ...
              <div className="mt-2 text-sm text-slate-200">
                timesteps: <span className="font-mono">{String(metrics.timesteps ?? metrics.total_timesteps ?? "—")}</span>
              </div>
              <div className="mt-1 text-sm text-slate-200">
                episodes: <span className="font-mono">{String(metrics.n_episodes ?? metrics.episodes ?? "—")}</span>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-950/30 p-4">
              ...
              <div className="mt-2 text-sm text-slate-200">
                metric: <span className="font-mono">{String(metrics.primary_metric ?? metrics.metric ?? "final_equity")}</span>
              </div>
              <div className="mt-1 text-sm text-slate-200">
                constraint: <span className="font-mono">{String(metrics.constraint ?? "drawdown-aware")}</span>
              </div>
            </div>
